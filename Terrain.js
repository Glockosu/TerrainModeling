...
console.log(colorRGBValues);

class Gradient {
  constructor(stops, resolution, customColors) {
    this.stops = {};
    ...
    stops.forEach((stop, index) => {
      ...
      // Replace color at the same index with the custom color if available
      if (customColors && customColors[index]) {
        this.stops[stop[0]] = nj.array(customColors[index]);
      }
    });
    this.colors = [];
  ...
  newGradient(r1, g1, b1, r2, g2, b2, r3, g3, b3, r4, g4, b4, r5, g5, b5) {
    gradient = new Gradient([0,    r1,  g1,  b1 ],  // brown
    [0.25, r2,  g2,  b2 ],  // dark green
    [0.60, r3, g3, b3 ],  // green
    [0.70, r4, g4, b4],  // tan
    [1, r5, g5, b5]   // white
    , 32)
...
// Function to store RGB values
function storeColorRGBValues() {
  // Clear the array before updating
  colorRGBValues.length = 0;

  // Loop through each color input and update the array
  for (let i = 1; i <= 5; i++) {
    const colorInput = document.getElementById(`color${i}`);
    const rgbValues = getRGBFromColorString(colorInput.value);

    // Store the RGB values in the array
    colorRGBValues.push(rgbValues);
  }

  // Log the array for checking
  console.log('Stored colorRGBValues:', colorRGBValues);
}
...
//      console.log(`queuing new chunk at ${i}, ${j}`)
      ...
      setTimeout(() => {
        ...
        let newChunk = new Chunk(
          ...
          offsetX, offsetZ, gradient.colors
        );
